 ...
 */
import { isDuplicate, parseCsv, readExport } from '../../../modules/vault/server/import.js';
...
console.log('\nChrome / Brave / Edge');
{
  const csv = [
    ...
    'Bank,https://bank.example/,sam@example.com,"p,a""ss",some note',
...
console.log('\nFirefox');
{
  const csv = [
    ...
    '"https://example.org","sam","secret",,"https://example.org","{abc}","1700000000"',
  ].join('\n');
  const result = readExport(csv);
...
console.log('\nduplicate detection');
const base = { title: '', username: 'sam', password: 'x', url: 'https://example.com/login', notes: '', totp: '' };
...
check('case in the username is ignored', isDuplicate(base, { ...base, username: 'SAM' }));
